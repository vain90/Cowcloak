...
OAuth browser login is not part of this suite because the disposable Mailcow has no configured external OAuth identity provider. The Rspamd check validates the real endpoint and response contract; generating representative mail events is intentionally left out until the CI fixture has a deterministic SMTP/Rspamd event source.
...
- on pull requests that change the integration-test infrastructure itself

It does not run for ordinary application or UI pull requests.
